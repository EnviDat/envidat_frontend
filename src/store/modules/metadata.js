...
*/
    for (let index = 0; index < metadataIds.length; index += 1) {
      const element = metadataIds[index];
      // for testing add all the same metadata details to the different metadata ids
      commit(ADD_TEST_METADATA, { id: element, data: dataset.result });
    }

    commit(LOAD_METADATAS_SUCCESS);
  },
  [GET_METADATA_BY_ID]({ commit }) {
    commit(GET_METADATA_BY_ID);
    // alert('before timeout');
    // setTimeout(() => {
    commit(GET_METADATA_BY_ID_SUCCESS, dataset.result);
    // alert('after timeout');
    // }, 2000);
  },
