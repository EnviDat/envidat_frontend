<template>
  <v-app class="application" :style="dynamicBackground">
    ...
    <navigation v-if="$vuetify.breakpoint.mdAndUp"
                :style="`z-index: ${NavigationZIndex}px`"
    ...
    <navigation-mobile v-if="$vuetify.breakpoint.smAndDown"
                    :navItems="navItems"
                    :style="`position: fixed; top: auto; right: 10px; bottom: 10px; z-index: ${NavigationZIndex}px`"
                    class="elevation-3"
                    @itemClick="catchItemClicked" />

    <navigation-toolbar :style="`z-index: ${NavToolbarZIndex}px`"
...
export default {
  ...
  data: () => ({
    ...
    showMenu: true,
    searchTerm: '',
    NavToolbarZIndex: 600,
    NavigationZIndex: 550,
    navItems: [
